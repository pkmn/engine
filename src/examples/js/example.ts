...
// @pkmn/engine does not export the PSRNG - PRNG from @pkmn/sim can be used if absolutely necessary,
// but instead an RNG optimized for JS should be used - here we're using a Mulberry32 RNG variant
// See also: https://github.com/bryc/code/blob/master/jshash/PRNGs.md
class Random {
  seed: number;

  // Slightly more sophisticated than https://xkcd.com/221/ because its output from a FNV-1a hash,
  // but in production one should really be seeding with crypto.getRandomBytes() or similar
...
// We could read this data in from a file (which even works with Parcel due to magic!),
// but for the sake of having an example that works without too much hairiness in
// both Node and the browser we inline these teams instead
...
//
// beforehand then we will simply run this example with the default configuration (-Dshowdown)
// and not receive any trace log messages. Similarly, `showdown: true` here only works because
// the default configuration opts into Pokémon Showdown compatibility mode - if we change
// around our configuration we will need to change this initialization option as well.
...
while (!(result = battle.update(c1, c2)).type) {
  // If -Dtrace is enabled we can parse and output the resulting logs since we initialized
  // the battle to support logging (both `-Dtrace` *and* `log: true` are required for logging)
  display();
