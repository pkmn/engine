...
const doMoveFns = (gen: Generation, dirs: { out: string; cache: string }, effects: Set<string>) => {
  if (gen.num !== 2) throw new Error(`Unexpected generation ${gen.num}`);

  const buf = ['//! Code generated by `tools/generate` - manual edits will be overwritten\n'];
  buf.push('const common = @import("../common/data.zig");\n');
  buf.push('const data = @import("data.zig");');
  buf.push('const mechanics = @import("mechanics.zig");\n');
  buf.push('const Player = common.Player;\nconst Result = common.Result;\n');
  buf.push('const Move = data.Move;\n');
  buf.push('const Effects = mechanics.Effects;\nconst State = mechanics.State;\n');
  const IMPORTS = [
    'checkHit', 'checkCriticalHit', 'calcDamage', 'adjustDamage', 'randomizeDamage',
    'applyDamage', 'buildRage', 'rageDamage', 'kingsRock', 'destinyBond',
  ];
  for (const command of IMPORTS) {
    buf.push(`const ${command} = mechanics.${command};`);
  }
  ...
  for (const effect of effects) {
    if (effect === 'HighCritical') continue; // covered below
    if (BOOST.test(effect)) {
      boosts.push(effect);
      continue;
    } else if (UNBOOST.test(effect)) {
      unboosts.push(effect);
      continue;
    }
    write(`.${effect} => {}, // TODO`);
  }
  let enums = boosts.sort().map(b => `.${b}`);
  write('// zig fmt: off');
  write(`${enums.slice(0, Math.floor(enums.length / 2)).join(', ')},`);
  write(`${enums.slice(Math.floor(enums.length / 2)).join(', ')} => {}, // TODO`);
  write('// zig fmt: on');
  enums = unboosts.sort().map(b => `.${b}`);
