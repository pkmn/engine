...
const doMoveFns = async (
  ...
  const IMPORTS = [
    ...
    'applyDamage', 'buildRage', 'kingsRock', 'destinyBond',
  ];
  ...
  const SNIPPETS: {[command: string]: (effect: string) => void} = {
    ...
    ragedamage: () => {
      newline();
      write('const volatiles = battle.side(player).active.volatiles;');
      write('assert(volatiles.Rage);');
      write('state.damage *|= (volatiles.rage +| 1);');
      newline();
    },
  };
