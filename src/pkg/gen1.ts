...
export class Battle {
  ...
  *sides() {
  ...
  static encode(
    ...
    Side.encode(gen, lookup, p1, data, OFFSETS.Battle.p1);
    Side.encode(gen, lookup, p2, data, OFFSETS.Battle.p2);
    const offset = OFFSETS.Battle.seed;
    if (seed.length === 4) {
  ...
  *pokemon() {
  ...
  static encode(
    ...
    for (let i = 0; i < 6; i++) {
      ...
      Pokemon.encode(gen, lookup, poke, data, off);
...
export class Pokemon {
  ...
  *moves() {
  ...
  get types(): Readonly<[TypeName] | [TypeName, TypeName]> {
...
function decodeTypes(lookup: Lookup, val: number): Readonly<[TypeName] | [TypeName, TypeName]> {
  const type1 = val & 0x0F;
  const type2 = val >> 4;
  return (type1 === type2
    ? [lookup.typeByNum(type1)]
    : [lookup.typeByNum(type1), lookup.typeByNum(type2)]);
}
